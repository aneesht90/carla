...
class CarlaGame(object):
    def __init__(self, carla_client, city_name=None, drive_mode=None):
    ...
    def _on_loop(self):
        ...
        if (self._drive_mode):
            ...
            self._get_drive_controller_control(pygame)
    ...
    def _test_drive_controller_mode(self, pygame):
        ...
        for j in range(0,pygame.joystick.get_count()):
            if pygame.joystick.Joystick(j).get_name() == WHEEL:
                ...
                #     print (event.button)
                #
                # print("pressed button is ",pygame.event.button)

                # buttons = wheel.get_numbuttons()
                # for i in range( buttons ):
                #     button = wheel.get_button( i )
                #     print("pressed button is", i, button)
            ...
            if event.axis == 0:
                ...
                if axis_mode ==1:
                    if event.value < 0:
                        ...
                        print("accelerator", pedal_value(event.value))
            elif event.axis == 2 and axis_mode == 2:
                print("brake", pedal_value(event.value))
            elif event.axis == 3:
                print("clutch", pedal_value(event.value))
    ...
    while True:
        try:
            ...
            with make_carla_client(args.host, args.port) as client:
                game = CarlaGame(client, args.map_name,drive_mode=args.driving_controller)
